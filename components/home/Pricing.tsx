...
const Pricing = ({ id, locale, langName }) => {
  return (
    <section id={id} className="py-16 px-4 sm:px-6 lg:px-8 bg-background text-foreground">
      ...
      <div className="flex flex-col sm:flex-row justify-center items-stretch gap-8">
        {locale.plans.map((plan, index) => (
          <Card key={index} className="bg-card shadow-lg rounded-lg overflow-hidden max-w-xs w-full flex flex-col justify-between">
            <div>
              ...
              <CardBody className="p-4 text-center">
                ...
                <ul className="space-y-2">
                  {(plan.features || []).map((feature, index) => (
                    <li key={index} className="flex items-center justify-center text-foreground">
